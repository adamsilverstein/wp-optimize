...
class WPO_Page_Cache {
	...
	public function enable($force_enable = false) {
		...
		// if WPO_CACHE isn't set, or advanced cache doesn't contain the right constant, force regeneration
	...
	public function disable() {
		...
		if (false === file_put_contents(untrailingslashit(WP_CONTENT_DIR) . '/advanced-cache.php', "<?php\n// WP-Optimize: page cache disabled")) {
	...
	private function write_advanced_cache() {
		...
		if (!isset($_SERVER['HTTP_HOST']) || (isset($_SERVER['HTTP_HOST']) && empty($_SERVER['HTTP_HOST']))) return false;
...
// WP-Optimize advanced-cache.php (do not change this line, it is used for correctness checks)
